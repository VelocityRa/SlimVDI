...
StoreFilename(CPFN pfn)
{
   ...
   if (pFnCache==NULL || (pFnCache->BytesUsed+alloc)>FN_CACHE_INCR) {
      FN_CACHE *p = (FN_CACHE*)Mem_Alloc(MEMF_ZEROINIT,sizeof(FN_CACHE));
      p->pNextCache = pFnCache;
      pFnCache = p;
   }
   rslt = (PFN)(pFnCache->buff + pFnCache->BytesUsed);
   memcpy(rslt, pfn, len);
...
AddToRegistry(CPFN fn, S_UUID *pUUID, FILETIME *ftLastModify)
{
   ...
   if (pmr) { // entry with this UUID exists already.
      if (CompareFileTime(ftLastModify,&pmr->ftLastModify)<0) {
         ...
         memcpy(&pmr->ftLastModify,ftLastModify,sizeof(FILETIME));
         pmr->filename = StoreFilename(fn);
      }
   } else {
      MED_REG_ENTRY mre;

      memcpy(&mre.uuid,pUUID,16);
      memcpy(&mre.ftLastModify,ftLastModify,sizeof(FILETIME));
