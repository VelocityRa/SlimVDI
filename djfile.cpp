...
File_Open(CPFN fn)
{
   FILE h = (FILE)CreateFile(fn,GENERIC_READ|GENERIC_WRITE,0,NULL,OPEN_EXISTING,0,0);
   if (h!=NULLFILE) IOR=0;
...
File_Create(CPFN fn, UINT flags)
{
   ...
   h = (FILE)CreateFile(fn,dwAccess,0,0,dwOpenMode,dwflags,0);
   if (h!=NULLFILE) IOR=0;
   else IOR = GetLastError();
   return h;
}
