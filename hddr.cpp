...
PUBLIC int
HDDR_ReadSectors(HVDDR pThis, void *buffer, HUGE_ LBA, UINT nSectors)
{
   VDDR_LastError = HDDR_ERR_INVHANDLE;
   if (pThis) {
      BYTE *pDest=buffer;
...
PUBLIC bool
HDDR_GetDriveSize(HVDDR pThis, HUGE_ *drive_size)
{
   if (pThis) {
      PHDD pHDD = (PHDD)pThis;
...
static UINT
BlockStatus(PHDD pHDD, HUGE_ LBA_start, HUGE_ LBA_end)
{
...
PUBLIC UINT
HDDR_BlockStatus(HVDDR pThis, HUGE_ LBA_start, HUGE_ LBA_end)
{
