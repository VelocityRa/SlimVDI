...
PUBLIC bool
VHDR_GetDriveSize(HVDDR pThis, HUGE_ *drive_size)
{
   if (pThis) {
...
// really apply to fixed VHDs, but we imagine they exist anyway...
{
   ...
   if (iPage<pVHD->nBlocks) {
      ...
      VDDR_LastError = VHDR_ERR_SEEK;
      OSLastError = DoSeek(pVHD->f, (((HUGE_)SID)+SectorOffset)<<9);
      if (OSLastError==0) {
         VDDR_LastError = VHDR_ERR_READ;
...
PUBLIC int
VHDR_ReadSectors(HVDDR pThis, void *buffer, HUGE_ LBA, UINT nSectors)
{
   VDDR_LastError = VHDR_ERR_INVHANDLE;
   if (pThis) {
      BYTE *pDest=buffer;
...
VHDR_GetDriveBlockCount(HVDDR pThis, UINT SPBshift)
{
   if (pThis) {
      PVHD pVHD = (PVHD)pThis;
      HUGE_ nBlocks = ((pVHD->ftr.u64CurrentSize>>9) + ((1<<SPBshift)-1))>>SPBshift;
      VDDR_LastError = 0;
      return LO32(nBlocks);
   }
   VDDR_LastError = VHDR_ERR_INVHANDLE;
...
static UINT
BlockUsed(PVHD pVHD, HUGE_ LBA_start, HUGE_ LBA_end)
{
   if (pVHD->ftr.u32DiskType==VHD_TYPE_DYNAMIC || pVHD->ftr.u32DiskType==VHD_TYPE_DIFFERENCING) {
...
PUBLIC UINT
VHDR_BlockStatus(HVDDR pThis, HUGE_ LBA_start, HUGE_ LBA_end)
{
   VDDR_LastError = VHDR_ERR_INVHANDLE;
   if (pThis) {
      VDDR_LastError = 0;
