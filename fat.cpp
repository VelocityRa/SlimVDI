...
typedef struct {   // not a literal image of the boot sector, instead a translation into more convenient form.
   HUGE_ FATBeginLBA;
   HUGE_ ClusterBeginLBA; // The area from ClusterBeginLBA to (ClusterBeginLBA+(nClusters*SectorsPerCluster)-1)
...
static bool
ReadBootSector(HVDDR hVDIsrc, FAT_BOOT_SECTOR *boots, HUGE_ iLBA)
{
   bool success = FALSE;
...
PUBLIC bool
FAT_IsFATVolume(HVDDR hVDI, HUGE_ iLBA)
{
   FAT_BOOT_SECTOR fbs;
...
PUBLIC HFSYS
FAT_OpenVolume(HVDDR hVDI, HUGE_ iLBA, HUGE_ cLBA, UINT cSectorSize)
{
