 ...
 */
{
   ...
   hInstRes = (HINSTANCE)hInstApp;
   
   dptr = GetModuleFileNameA((HMODULE)hInstApp,argbuff,MAX_PATH)+1;
   argv[0] = argbuff;
...
PUBLIC bool
Env_GetDiskFreeSpace(CPFN szPath, HUGE_ *freebytes)
{
...
Env_LoadBinData(PSTR szResource)
{
   ...
   if (hRes) { // am I supposed to free the hRes handle?
      HGLOBAL hData = LoadResource(hInstRes,(HRSRC)hRes);
      if (hData) {
   ...
   if (*psz == NULL) {
      ...
      if (pwsz==NULL || pwsz[0]==0) *psz = "";
         ...
         if (pStrCache==NULL || (pStrCache->BytesUsed+alloc)>STR_CACHE_INCR) {
            STRING_CACHE *p = (STRING_CACHE*)Mem_Alloc(MEMF_ZEROINIT,sizeof(STRING_CACHE));
...
Env_GetTempFileName(PSTR path, PSTR pszPrefix, UINT uUnique, PSTR pszOutputFilename)
{
   ...
   if (!path) {
      GetTempPath(1024,(LPSTR)szTempPath);
      path = (PSTR)szTempPath;
   }
