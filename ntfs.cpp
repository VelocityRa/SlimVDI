...
static bool
ReadClusters(PNTFSVOL pNTFS, PVOID dest, HUGE_ LCN, UINT nClusters)
{
   HUGE_ LBA;
...
static void
UnpackBootSector(NTFS_BOOT_SECTOR *boots, BYTE *buffer, HUGE_ iLBA, HUGE_ cLBA)
{
...
PUBLIC bool
NTFS_IsNTFSVolume(HVDDR hVDI, HUGE_ iLBA)
{
...
PUBLIC HFSYS
NTFS_OpenVolume(HVDDR hVDI, HUGE_ iLBA, HUGE_ cLBA, UINT cSectorSize)
{
...
static UINT
BinLength(HUGE_ n) // Returns the number of bytes needed to store the SI64 number 'n'
{
   UINT L=0,lsb=0;
   if (n==0) return 0;
   do {
