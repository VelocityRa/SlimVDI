...
PUBLIC bool
VMDKR_GetDriveSize(HVDDR pThis, HUGE_ *drive_size)
{
   if (pThis) {
...
// is a sector offset within the block selected by "iPage". Length is in bytes.
{
   ...
   if (iPage<pe->nBlocks) {
      ...
      if (pe->buff) {
         ...
         VDDR_LastError = VMDKR_ERR_SEEK;
         OSLastError = DoSeek(pe->f, ((HUGE_)SID)<<9);
         if (OSLastError==0) {
            VDDR_LastError = VMDKR_ERR_READ;
...
PUBLIC int
VMDKR_ReadSectors(HVDDR pThis, void *buffer, HUGE_ LBA, UINT nSectors)
{
   VDDR_LastError = VMDKR_ERR_INVHANDLE;
   if (pThis) {
      BYTE *pDest=buffer;
...
static UINT
BlockStatus(PVMDK pVMDK, HUGE_ LBA_start, HUGE_ LBA_end)
...
PUBLIC UINT
VMDKR_BlockStatus(HVDDR pThis, HUGE_ LBA_start, HUGE_ LBA_end)
{
   VDDR_LastError = VMDKR_ERR_INVHANDLE;
   if (pThis) {
      VDDR_LastError = 0;
