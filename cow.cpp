...
typedef struct {
   ...
   HUGE_  DiskSize;    // total size of virtual disk (sum of all extent sizes, in sectors).
...
static EXTREF
FindExtent(PCOW pCOW, HUGE_ LBA, HUGE_ *LBA_out)
...
PUBLIC bool
COW_GetDriveSize(HVDDR pThis, HUGE_ *drive_size)
{
   if (pThis) {
...
static UINT
BlockStatus(PCOW pCOW, HUGE_ LBA_start, HUGE_ LBA_end)
...
PUBLIC UINT
COW_BlockStatus(HVDDR pThis, HUGE_ LBA_start, HUGE_ LBA_end)
{
   VDDR_LastError = COW_ERR_INVHANDLE;
   if (pThis) {
      VDDR_LastError = 0;
...
static int
RawReadSectors(PCOW pCOW, PEXTENT pe, BYTE *buffer, HUGE_ LBA, UINT offset_sector, UINT nSectors)
...
PUBLIC int
COW_ReadSectors(HVDDR pThis, void *buffer, HUGE_ LBA, UINT nSectors)
{
   VDDR_LastError = COW_ERR_INVHANDLE;
   if (pThis) {
      BYTE *pDest=buffer;
...
static EXTREF
SplitExtent(PCOW pCOW, HUGE_ LBA)
...
static EXTREF
CopyToCOW(PCOW pCOW, HUGE_ LBA, UINT nSectors)
...
{
   HUGE_ LBA_cowstart = ((LBA>>COW_SPB_SHIFT)<<COW_SPB_SHIFT);
   HUGE_ LBA_cowend;
...
PUBLIC int
COW_WriteSectors(HCOW pThis, void *buffer, HUGE_ LBA, UINT nSectors)
{
   VDDR_LastError = COW_ERR_INVHANDLE;
   if (pThis) {
      HUGE_ LBA_cowstart;
...
PUBLIC int
COW_InsertSectors(HCOW pThis, HUGE_ LBA, UINT nSectors)
{
   VDDR_LastError = COW_ERR_INVHANDLE;
   if (pThis) {
      PCOW pCOW = (PCOW)pThis;
...
PUBLIC int
COW_MoveSectors(HCOW pThis, HUGE_ LBA_from, HUGE_ LBA_to, UINT nSectors)
{
   VDDR_LastError = COW_ERR_INVHANDLE;
   if (pThis) {
      PCOW pCOW = (PCOW)pThis;
