...
typedef struct {
   ...
   HUGE_ cDriveSectors;
   UINT nPartitions;
   HUGE_ FreeSpaceLBA; // LBA of first sector beyond end of last partition.
   HUGE_ PartStart[MAX_PARTITIONS];
   HUGE_ PartSectors[MAX_PARTITIONS];
} UNPARTINF, *PUNPART;

/*.....................................................*/

static bool
LargeUnallocSpace(PUNPART pUnpart, BYTE *MBR, HUGE_ cDriveSectors)
{
   bool bBigFreeSpace = FALSE;
   if (MBR[510]==0x55 && MBR[511]==0xAA) {
      UINT i,j,cLBA;
      PPART pPart;
      HUGE_ startLBA,prevEnd;
      bool IsLDM = FALSE;
      ...
      pPart = (PPART)(MBR+446);
      for (i=0; i<4; i++,pPart++) {
         cLBA = (HUGE_)MAKELONG(pPart->loNumSectors,pPart->hiNumSectors);
...
PUBLIC HFSYS
Unpart_OpenVolume(HVDDR hVDI, HUGE_ iLBA, HUGE_ cLBA, UINT cSectorSize)
{
...
Unpart_IsBlockUsed(HFSYS hUnpart, UINT iBlock, UINT SectorsPerBlockShift)
{
   if (hUnpart) {
      ...
      if (LBA>=pUnpart->FreeSpaceLBA) { // check if free space falls beyond last partition - also works if no partitions are defined
         ...
         HUGE_ FreeSpaceLBA = pUnpart->PartStart[0]+pUnpart->PartSectors[0]; // start of first gap.
         HUGE_ BlockEndLBA = LBA+(HUGE_)(1<<SectorsPerBlockShift);
