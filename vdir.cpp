      ...
      if (File_RdBin(f,&vph,sizeof(vph))==sizeof(vph)) {
         if (PreHeaderOK(&vph)) {
            ...
            if (bOK) {
               ...
               if (bOK) {
                  pVDI = (PVDI)Mem_Alloc(MEMF_ZEROINIT,sizeof(VDI_INFO));
                  pVDI->f = f;
                  ...
                  if (!VDDR_LastError) {
                     ...
                     pVDI->blockmap = (UINT*)Mem_Alloc(0, mapsize);
                  ...
                  if (VDDR_LastError) pVDI = (PVDI)Mem_Free(pVDI);
               }
            }
         }
      }
...
// but is expected to be a multiple of the sector size.
{
   ...
   if (iPage<pVDI->hdr.nBlocks) {
      ...
      if (SID == VDI_PAGE_FREE) {
         ...
         seekpos = (((HUGE_)SID)<<pVDI->BlockShift);
...
PUBLIC int
VDIR_ReadSectors(HVDDR pThis, void *buffer, HUGE_ LBA, UINT nSectors)
{
   VDDR_LastError = VDIR_ERR_INVHANDLE;
   if (pThis) {
...
PUBLIC bool
VDIR_GetDriveSize(HVDDR pThis, HUGE_ *drive_size)
{
   if (pThis) {
      PVDI pVDI = (PVDI)pThis;
...
VDIR_GetDriveBlockCount(HVDDR pThis, UINT SPBshift)
{
   if (pThis) {
      PVDI pVDI = (PVDI)pThis;
      HUGE_ nBlocks = pVDI->hdr.nBlocks;
...
static UINT
BlockStatus(PVDI pVDI, HUGE_ LBA_start, HUGE_ LBA_end)
{
...
PUBLIC UINT
VDIR_BlockStatus(HVDDR pThis, HUGE_ LBA_start, HUGE_ LBA_end)
{
